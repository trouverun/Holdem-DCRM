...
class EvaluatorNetwork(torch.nn.Module):
    def __init__(self, device, bet_buckets=N_BET_BUCKETS):
        ...
        self.bet_buckets = bet_buckets
        self.rnn = torch.nn.GRU(OBS_SHAPE, RNN_HIDDENS, 1, batch_first=True)
        self.value = torch.nn.Linear(RNN_HIDDENS, 1)
        self.action_logits = torch.nn.Linear(RNN_HIDDENS, N_ACTIONS)
        self.bet_logits = torch.nn.Linear(RNN_HIDDENS, self.bet_buckets)
